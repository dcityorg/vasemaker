...
│   ├── svg-rasterizer.ts # SVG → grayscale pixels via canvas (browser-only, used by use-vase-mesh.ts). setSvgSize() strips width/height including units (px, em, %) before injecting target dimensions
...
- **SVG Pattern texture** — SVG as radial displacement. Three buttons: **Load SVG** (file picker for `.svg` files), **Save SVG** (downloads clean `.svg` for reuse), **Paste SVG** (dialog for pasting SVG code or CSS code from pattern sites). **Transform buttons** next to thumbnail: Rotate 90° CW (cycles 0→90→180→270), Flip Horizontal, Flip Vertical — transforms stored as `rotation`/`flipX`/`flipY` in params, applied as UV remapping in `sampleSvgPattern()`. Thumbnail shows single tile with transforms applied. Status indicator shows "SVG loaded" / "SVG empty". Paste dialog has Clear button to reset textarea before pasting new content. Accepts raw SVG markup `<svg>...</svg>`, data URLs, and CSS `background-image: url(...)` lines. Parser handles raw SVG with parentheses inside `url()` (Pattern Monster), percent-encoded `%23` → `#` references, and `width='100%'` percentage dimensions (stripped and replaced with pixel values). Modal dialog with tiled preview, aspect-ratio-preserving rasterization via offscreen canvas, Gaussian blur anti-aliasing, bilinear interpolation sampling. Architecture: `svg-rasterizer.ts` (browser-only DOM) rasterizes to grayscale `Uint8Array`; `svg-pattern.ts` stores pixel data at module level via `setSvgPatternData()` setter (keeps it DOM-free); `use-vase-mesh.ts` bridges async rasterization to synchronous mesh rebuild via version counter. Higher mesh resolution (100+ vertical/radial) recommended for fine patterns. **SVG motif placement controls (v0.902):** # Tiles Around/Vert (count), SVG Size X/Y (10–250%, motif size within cell, 100%=tiling), Rotate (0–360° uniform), Rand. Rotate/Size (per-tile random with separate ↻ re-roll buttons and independent seeds), Mirror Alt. (flip alternate tiles), Vert Spacing (row gap as % of vase height, 0=auto), Vert Move (shift rows up, clips at top/bottom instead of wrapping), Stagger Rows (brick-pattern offset for alternate rows). `computeSvgTileCoords()` in `textures.ts` is the shared helper for both displacement and cutout; returns `[tileU, tileV, fadeFactor]` where `fadeFactor` smoothly transitions from 1→0 at motif edges (5% smoothstep fade zone, controlled by `fadeU`/`fadeV` constants in the function — reduce to ~1-2% for sharper edges or increase for softer). SliderRow supports optional `suffix` prop for units display
- **Wall thickness, base cap, rim** — Outer + inner surfaces, base cap (no wall strip — avoids lip on flared profiles), flat/rounded rim. Base thickness measured vertically. **Smooth Inner** toggle keeps inner wall completely smooth (no textures) via `skipEffects=true`; **Min Wall** slider prevents paper-thin walls where effects indent inward. **Open bottom (base thickness = 0):** When `bottomThickness` is 0 and `wallThickness > 0`, a bottom ring annulus at z=0 connects the outer and inner walls to create a manifold mesh. This allows bottomless vases (e.g. for LED candles) — the slicer fills between the walls with plastic for structural strength. **Rim anti-overhang:** In `mesh-generator.ts` section 2b, after computing outer/inner wall vertices, the top row is clamped so it never flares out past the row below (prevents rim overhang when textures have less displacement at the very top, e.g. SVG grass with white sky at top of tile). Both flat and rounded rims read from the clamped wall buffer instead of recomputing positions independently
...
- **Presets** — Custom dropdown with thumbnail previews (images in `public/presets/`), preset name, and description. 8 built-in presets. App starts with Classic Vase preset (in blue). Voronoi texture hidden from UI but engine code preserved for future improvements
...
## Stipple Texture (engine: `woodGrain`) — Iteration Notes

The current Stipple texture (formerly "Carved Wood", v1) uses simplex-perturbed sine waves. It produces organic-looking patterns but NOT realistic flat-sawn wood grain with thin delicate vertical lines. Multiple algorithm approaches were tried:
