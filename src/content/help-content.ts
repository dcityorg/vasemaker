...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  // ─── 3. Profile, Twist & Sway ─────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'keyvalue', items: [
        ...
        { key: '▲▼ buttons', value: 'Profile only: step the selection to the next point up/down — the way to grab points that crowd or overlap each other. With nothing selected, re-selects the last point' },
  ...
  // ─── 4. Textures ──────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'keyvalue', items: [
        ...
        { key: 'SVG Padding', value: 'Virtual white canvas added around the SVG bitmap at rasterization time (in pixels, 0\u201330). Use this when the SVG artwork runs right to the viewBox edge and produces small vertical divots or cliffs at motif boundaries on the vase. 3\u201310 px is usually enough. Saves you from having to open Affinity / Inkscape / Illustrator to re-save the SVG with margin. Uniform on all sides, independent of motif size' },
        { key: '# Tiles Around', value: 'Number of pattern tiles around the circumference' },
        { key: '# Tiles Vert', value: 'Number of pattern tile rows up the height (fractional values OK)' },
        { key: 'SVG Size X / Y', value: 'Motif size as % of tile cell. 100% = seamless tiling (edge content in neighboring tiles meets and merges). Below 100% = flat gutter between motifs; displacement stops cleanly at the motif boundary. Above 100% = motif stretches beyond the cell (useful with Vert Move to extend coverage). Reset button sets both to 100%. Tip: if your SVG has content right at the viewBox edge, use 100% to let tiles merge, or ≤ ~90% so the gutter is clearly wider than the depth \u2014 the 91\u201399% range can show a visible rim where motifs almost-touch' },
        { key: 'Rotate', value: 'Uniform rotation for all tiles (0\u2013360\u00b0)' },
        { key: 'Rand. Rotate', value: 'Max random tilt per tile. \u21bb button re-rolls the random pattern' },
        { key: 'Rand. Size', value: 'Max random size variation per tile. \u21bb button re-rolls independently' },
        { key: 'Mirror Alt.', value: 'Flip every other tile horizontally (checkerboard mirror pattern)' },
        { key: 'Vert Spacing', value: 'Distance between row centers (% of vase height). 0 = auto (rows evenly divided). Increase to spread rows apart, decrease to pack them closer' },
        { key: 'Vert Move', value: 'Shift all rows up/down the vase (% of vase height, -50% to 100%). Negative values shift down. Use with SVG Size Y > 100% to stretch the motif to cover the full vase' },
        { key: 'Around Move', value: 'Shift all tiles around the circumference (% of cell width). Useful for positioning motifs on flat faces of non-circular shapes like Square or Rectangle' },
        { key: 'Stagger Rows', value: 'Shift alternate rows horizontally (% of cell width). 50% gives a brick-like offset' },
        { key: 'Invert SVG', value: 'Swap which areas are grooves vs. ridges' },
        { key: 'Raised', value: 'When on, pattern protrudes outward instead of indenting inward. Eliminates the lip at tile edges that occurs when using Invert on mostly-white SVGs. Depth slider becomes Height' },
        { key: 'Depth / Height', value: 'How far the pattern displaces (mm). Label shows Depth when indenting, Height when Raised is on' },
      ...
      { type: 'tip', text: 'Cleaning up edge artifacts: if you see faint stepping or staircase patterns at motif boundaries, try nudging Vertical or Radial Resolution up or down by a small amount (e.g. 200 \u2192 201, 360 \u2192 363). Small resolution changes shift where the mesh grid samples the SVG, which can break up regular aliasing patterns against your motif edges.' },
      { type: 'heading', text: 'Give Your SVG Some Canvas Padding' },
      { type: 'paragraph', text: 'If your SVG artwork runs right up to the edge of its canvas (viewBox), you may see thin vertical divots or ridges on the vase near the sharpest parts of the motif \u2014 leaf tips, crescent points, letter corners. This happens because the engine returns the vase surface to its baseline radius cleanly outside the motif area, which creates a tiny cliff wherever dark SVG content meets the motif boundary.' },
      { type: 'paragraph', text: 'Fastest fix: use the SVG Padding slider (near the top of the SVG Pattern controls, just above the "SVG loaded" status). Dial 3\u201310 px and the divots disappear \u2014 no need to open another app. The slider adds a virtual white border around the SVG bitmap at rasterization time, which is exactly what baking extra canvas into the SVG file would do.' },
      { type: 'paragraph', text: 'If you\u2019d rather fix the SVG file itself (e.g. so the padding travels with the asset to other tools), add 5\u201310 pixels of empty canvas on all four sides of the artwork. Do this once per SVG and the fix is permanent.' },
      { type: 'list', items: [
        'Affinity Designer: Document \u2192 Resize Document, check "Rescale" off, add ~10px to width and height, then recenter the artwork (or offset the canvas by half the added amount)',
        'Inkscape: File \u2192 Document Properties \u2192 Resize to content, then add a small margin value',
        'Adobe Illustrator: File \u2192 Document Setup, or use Object \u2192 Artboards \u2192 Artboard Options to expand',
        'Boxy SVG, SVG-Edit, or any other vector editor with a canvas/viewBox resize command',
        'Text editor (for simple SVGs with one path): edit the viewBox attribute directly \u2014 e.g. change viewBox=\"0 0 435 698\" to viewBox=\"-10 -10 455 718\" (expand by 10 on each side); the path inside doesn\'t need to move',
      ] },
      { type: 'tip', text: 'If you built your own SVG in a design tool, leave some empty canvas around the artwork from the start. 5\u201310% of the artwork\u2019s size is a good default and it costs nothing.' },
  ...
  // ─── 5. 3D Printing Tips ──────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'heading', text: 'Inner Wall — Radial vs Parallel' },
      ...
      { type: 'keyvalue', items: [
        { key: 'Radial', value: 'Inner offset along the radius from the central axis. Fast and stable, but the perpendicular wall thickness shrinks near corners — a 1×2 Rectangle with 2mm wall has only ~1mm of actual wall at its corners.' },
        { key: 'Parallel', value: 'Inner surface stays parallel to the outer wall (offset perpendicular to the outer surface). Produces a true parallel curve so the wall is uniformly thick everywhere. Recommended for Rectangle, Square, Polygon, Ellipse.' },
      ] },
      { type: 'tip', text: 'Default is Radial to preserve behavior of existing designs. Switch to Parallel whenever you\'re printing a non-circular box or tray and want consistent wall thickness all the way around. Strongly concave shapes (Cardioid, Heart, Butterfly) may show overlapping inner geometry at large wall thickness in Parallel mode — stick with Radial there.' },
  ...
  // ─── 6. MoldMaker ─────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      { type: 'keyvalue', items: [
        ...
        { key: 'Foot Recess', value: 'Recesses the master\'s bottom in printable steps so every cast piece gets a foot ring and a recessed center (a glaze well). Foot Width = the flat outer ring, Slope Width = the stepped ramp, Depth = how far the center is raised. Set Step Height to your printer\'s layer height for clean steps. Turn on Smooth Inside to keep surface texture out of the ramp and recessed center.' },
      ] },
      { type: 'paragraph', text: 'The cottle follows the full shape of your vase — ripples, squares, any cross-section — so its rim always matches the master\'s lid. Its floor has a 4mm center hole to let air in when pulling the set plaster block out, and its interior floor is never smaller than 40mm across so the plaster block always has a flat base to stand on.' },
      { type: 'paragraph', text: 'The Plaster Estimate section shows how much plaster the mold needs, split into approximate powder and water weights for the chosen plaster type. The Printer Fit section shows each part\'s widest diameter and height so you can check they fit your printer.' },
