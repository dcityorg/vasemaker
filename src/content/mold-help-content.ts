...
export const MOLD_HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'paragraph', text: 'Both the flange rings and the vertical seams use the same V ridge-and-groove profile as the handle mold, which poured completely leak-free. The two vertical Vs on each seam fin sit BETWEEN the two flange rings (alternating ring, V, ring, V going outward) and run all the way down to the center’s flange — through the flange band they continue as ridge posts on one half seated in matching channels in the other. Plaster escaping in any direction has to climb two barriers at every height — through the wall seam, through the fin seam, or out under the flange. A loose fit still seals; the clips do not need to be tight.' },
